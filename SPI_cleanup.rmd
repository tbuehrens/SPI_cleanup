...
finaldf<-dat%>%
  left_join(dups%>%
               ...
               ,join_by(commonpopname,spawningyear))%>%
  filter(is.na(dup))%>%
  dplyr::select(commonpopname,spawningyear,popfit,popfitnotes,tsaej,tsaij,nosaej,nosaij,phosej,phosij)%>%
...
```{r}
read_csv("data/tbl_SPI_stock_cleaned.csv")%>%
  dplyr::select(esu_dps_name=nosa_esu_dps_name,nosa_esa_pop_name,commonpopname=stock_name,stock_id)%>%
  right_join(finaldf, join_by(commonpopname))%>%
  write.csv(finaldf,paste0("SPI_dat_",Sys.Date(),".csv"),row.names = F)
```
